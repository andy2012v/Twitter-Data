...
#1
print "QUESTION #1:""\n""\n"
twitter_data.each do |key|
  key.each do |value, info|
    print "#{value}: "
    if info["description"].nil?
      puts "NA"
     elsif info.has_key?("description")
...
twitter_data.each do |key|
  key.each do |value, info|
    print "#{value}: "
    if info.has_key?("number of followers")
       puts info["number of followers"]
     end
    end
  end
...
twitter_data.each do |key|
  key.each do |value, info|
    print "#{value} "
    if info.has_key?("latest tweet")
      info["latest tweet"].size
       puts "latest tweet was #{info["latest tweet"].size} characters long."
     end
    end
  end
puts
...
twitter_data.each do |key|
  key.each do |value, info|
    print "#{value} "
    if info.has_key?("last twenty tweets")
      count = 0
      info["last twenty tweets"].each {|s| count += s.length}
      puts count
     end
    end
  end
...
# Note
# USE THIS TO FIND THE SIZE OR CHARACTER TOTALS OF ELEMENTS IN AN ARRAY!!!!!!!
...
# 1
users = Array.new []
total_followers = Array.new []
twitter_data.each do |data|
  data.each do |value, info|
    users << value
    total_followers << info["number of followers"]
  end
end
sorted_followers = total_followers.sort_by { |number| -number }

correct_user = sorted_followers.max
position = total_followers.index(correct_user)
person = users[position]

puts "The user #{person}, has the most followers at #{correct_user}."
...
# 2.
users = Array.new []
total_friends = Array.new []
twitter_data.each do |data|
  data.each do |value, info|
    users << value
    total_friends << info["number of friends"]
  end
end
sorted_friends = total_friends.sort_by { |number| -number }

correct_user = sorted_friends.max
position = total_friends.index(correct_user)
person = users[position]
...
twitter_data.each do |data|
  data.each do |value, info|
    users << value
    total_tweets << info["number of tweets"]
  end
end
sorted_tweets = total_tweets.sort_by { |number| -number }

correct_user = sorted_tweets.max
position = total_tweets.index(correct_user)
person = users[position]
